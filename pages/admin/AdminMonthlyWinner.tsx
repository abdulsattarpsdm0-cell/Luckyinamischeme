...
const AdminMonthlyWinner: React.FC = () => {
  ...
  const monthlyPlans = lotteryPlans.filter(p => p.drawCycle === 'MONTHLY');
  const selectedPlan = monthlyPlans.find(p => p.id === activeCategory);
  ...
  return (
    <div className="space-y-8 md:space-y-12 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tight">Monthly Winner Draws</h1>
          <p className="text-xs md:text-sm text-slate-500 font-medium italic">"Luck starts here - Live draw management for monthly plans."</p>
        </div>
        <button className="flex items-center space-x-2 bg-pink-50 px-4 py-2 rounded-xl border border-pink-100 text-[10px] font-black uppercase tracking-widest text-pink-600">
          <History size={16} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
        {monthlyPlans.map((plan) => {
          ...
          return (
            ...
            >
              ...
              <div className="absolute bottom-6 right-6 w-10 h-10 bg-pink-600 rounded-xl flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0 shadow-lg">
        ...
        {monthlyPlans.length === 0 && (
          <div className="col-span-full p-10 text-center bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2rem]">
            <p className="text-slate-400 font-black uppercase tracking-widest text-xs">No monthly plans found</p>
          </div>
        )}
      </div>
              ...
              <div className="flex-grow overflow-y-auto p-6 md:p-10 space-y-8">
                 <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                       <button onClick={() => setDrawMethod('MANUAL')} className={`p-4 md:p-6 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 ${drawMethod === 'MANUAL' ? 'border-pink-600 bg-pink-50 text-pink-700' : 'border-slate-100 text-slate-400'}`}>
                          <Target size={20} />
                       ...
                       {Array.from({ length: Math.min(selectedPlan.totalWinners, 10) }).map((_, i) => (
                         <div key={i} className="relative">
                            ...
                            <input 
                              ...
                              className="w-full h-12 md:h-16 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:border-pink-600 text-center font-black text-lg"
                            />
                         </div>
                       ))}
                    </div>
...
export default AdminMonthlyWinner;
