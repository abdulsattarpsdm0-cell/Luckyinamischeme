...
const AdminWeeklySoldTokens: React.FC = () => {
        ...
        {plansList.map(plan => {
          ...
          return (
            <button 
              ...
              className={`p-4 rounded-2xl border shadow-sm flex flex-col items-center text-center group transition-all ${isSelected ? 'bg-indigo-50 border-indigo-300' : 'bg-white border-slate-100 hover:border-indigo-200'}`}
            >
               <span className="text-xl mb-1 group-hover:scale-110 transition-transform">{plan.icon}</span>
               <span className={`text-[8px] font-black uppercase tracking-tighter truncate w-full ${isSelected ? 'text-indigo-700' : 'text-slate-400'}`}>{plan.name}</span>
               <span className={`text-lg font-black leading-none mt-1 ${isSelected ? 'text-indigo-900' : 'text-slate-900'}`}>{count}</span>
            </button>
          );
        })}
      </div>
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-6 md:space-y-10 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6 px-1">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tight">Weekly Sold Tokens</h1>
          <p className="text-xs md:text-sm text-slate-500 font-medium mt-1">Categorized view of all active sales for Weekly draws.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
        <div className="bg-slate-900 p-5 md:p-6 rounded-2xl md:rounded-[2.5rem] text-white flex items-center space-x-4 md:space-x-5 shadow-2xl relative overflow-hidden">
           ...
           <div className="min-w-0">
              <span className="block text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1 truncate">Total Weekly Sales</span>
              <span className="text-2xl md:text-3xl font-black truncate">{weeklyTokens.length}</span>
           </div>
        ...
        <div className="bg-white p-5 md:p-6 rounded-2xl md:rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center space-x-4 md:space-x-5">
           ...
           <div className="min-w-0">
              <span className="block text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1 truncate">Active Weekly Plans</span>
              <span className="text-2xl md:text-3xl font-black text-slate-900 truncate">{weeklyPlans.length}</span>
           </div>
        </div>
      </div>
...
export default AdminWeeklySoldTokens;
