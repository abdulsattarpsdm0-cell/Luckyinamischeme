...
import { LotteryPlan } from '../types.ts';
...
const LotteriesPage: React.FC = () => {
  ...
  const weeklyPlans = useMemo(() => filteredPlans.filter(p => p.drawCycle === 'WEEKLY'), [filteredPlans]);
  const monthlyPlans = useMemo(() => filteredPlans.filter(p => p.drawCycle === 'MONTHLY'), [filteredPlans]);

  const renderPlanCard = (plan: LotteryPlan) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 animate-in fade-in duration-500">
      ...
      {weeklyPlans.length > 0 && (
        <div className="mb-16">
          <div className="flex items-center space-x-3 mb-8">
            ...
            <div>
              <h2 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tight">Weekly Draws</h2>
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Har hafte jeetne ka mauqa</p>
            </div>
          </div>
      ...
      {monthlyPlans.length > 0 && (
        <div className="mb-16">
          <div className="flex items-center space-x-3 mb-8">
            ...
            <div>
              <h2 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tight">Monthly Draws</h2>
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Bade inamaat har mahine</p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10">
      ...
      {filteredPlans.length === 0 && (
        <div className="py-16 md:py-24 bg-slate-50 rounded-[2.5rem] md:rounded-[4rem] border border-dashed border-slate-200 text-center flex flex-col items-center">
